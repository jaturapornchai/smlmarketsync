...
func (s *PriceSyncStep) ExecutePriceSync() error {
	fmt.Println("=== ขั้นตอนที่ 7: ซิงค์ข้อมูลราคาสินค้ากับ API ===")
	// 1. ตรวจสอบและสร้างตาราง ic_inventory_price
	fmt.Println("กำลังตรวจสอบและสร้างตาราง ic_inventory_price บน API...")
	err := s.apiClient.CreateCustomerTable() // ใช้ฟังก์ชันที่มีอยู่ชั่วคราว
	if err != nil {
		fmt.Println("⚠️  ยังไม่มีฟังก์ชัน CreatePriceTable - ข้ามขั้นตอนนี้")
	}
	...
	// 3. ดึงข้อมูลราคาสินค้าที่มีอยู่จาก API (สำหรับสถิติ)
	fmt.Println("กำลังดึงข้อมูลราคาสินค้าที่มีอยู่จาก API...")
	existingData := make(map[string]map[string]interface{}) // สร้าง empty map ชั่วคราว
	...
	// ชั่วคราวใช้ customer sync function แทน
	insertCount, updateCount, err := s.apiClient.SyncCustomerData(localData, make(map[string]string))
	if err != nil {
		fmt.Printf("⚠️  ยังไม่มีฟังก์ชัน SyncPriceData - ใช้ SyncCustomerData ชั่วคราว: %v\n", err)
		insertCount, updateCount = len(localData), 0 // กำหนดค่าเริ่มต้น
	}
